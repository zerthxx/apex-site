...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    .from("tasks")
    .select(
      `id, title, description, due_date, priority, status, assigned_to, project_id, created_at, projects(id, name)`,
    )
    .is("deleted_at", null)
...
export async function POST(req: NextRequest) {
  ...
  if (error)
    return NextResponse.json({ error: error.message }, { status: 500 });
  await logActivity(supabase, user.id, "task_created", {
    task_id: data.id,
    title: data.title,
  });
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "id vaaditaan" }, { status: 400 });
  const { error } = await softDelete(supabase, "tasks", id);
  if (error)
    return NextResponse.json({ error: error.message }, { status: 500 });
