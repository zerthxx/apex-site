...
import { softDelete, cascadeSoftDeleteCustomer } from "@/lib/softDelete";
...
export async function GET(
  ...
  if (customerRes.error)
    return NextResponse.json({ error: "Asiakasta ei löydy" }, { status: 404 });
...
export async function DELETE(
  ...
  if (error)
    return NextResponse.json({ error: error.message }, { status: 500 });
  await cascadeSoftDeleteCustomer(supabase, id);
