...
import { softDelete, cascadeSoftDeleteProject } from "@/lib/softDelete";
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await softDelete(supabase, "projects", id);
  if (!error) await cascadeSoftDeleteProject(supabase, id);
  if (error)
    return NextResponse.json({ error: error.message }, { status: 500 });
