  ...
  async function submit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      start_at: form.all_day
        ? `${form.start_at.slice(0, 10)}T00:00:00Z`
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <button
            onClick={prevMonth}
            className="p-1.5 rounded-lg hover:bg-surface text-ink-ghost hover:text-ink transition-colors"
          >
            <ChevronLeft size={17} />
          </button>
          <h2 className="text-base font-semibold text-ink min-w-[180px] text-center">
            {MONTHS_FI[month]} {year}
          </h2>
          <button
            onClick={nextMonth}
            className="p-1.5 rounded-lg hover:bg-surface text-ink-ghost hover:text-ink transition-colors"
      ...
      {view === "month" ? (
        <div className="bg-elevated border border-wire rounded-xl overflow-hidden">
          ...
          <div className="grid grid-cols-7 border-b border-wire">
            {WEEKDAYS.map((d) => (
              <div
                key={d}
                className="py-2 text-center text-xs font-semibold text-ink-ghost uppercase tracking-wider"
              >
                {d}
              </div>
            ))}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    {dayEvents.length > 2 && (
                      <div className="text-[10px] text-ink-ghost px-1">
                        +{dayEvents.length - 2}
                      </div>
                    )}
                  </div>
        ...
        <div className="flex flex-col gap-2">
          {sortedEvents.length === 0 ? (
            <div className="bg-elevated border border-wire rounded-xl py-16 text-center text-sm text-ink-ghost">
              Ei tapahtumia
            </div>
          ) : (
            sortedEvents.map((e) => (
              <div
                key={e.id}
                className="flex items-start gap-3 p-4 bg-elevated border border-wire rounded-xl"
              >
                <div
                  className={cn(
                    "w-1 self-stretch rounded-full shrink-0",
                    TYPE_DOT[e.type] ?? "bg-ink-ghost",
                  )}
                />
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-ink-ghost mt-1.5">
                    {e.all_day
                      ...
                      : new Date(e.start_at).toLocaleString("fi-FI", {
                          dateStyle: "short",
                          timeStyle: "short",
                        })}
                    {e.end_at &&
                      !e.all_day &&
                      ` – ${new Date(e.end_at).toLocaleTimeString("fi-FI", { hour: "2-digit", minute: "2-digit" })}`}
                  </p>
                </div>
              </div>
            ))
      ...
      {eventToDelete && (
        <ConfirmDialog
          ...
          message={`Tapahtuma "${eventToDelete.title}" siirretään roskakoriin. Vain omistaja voi palauttaa tai poistaa sen pysyvästi.`}
