...
export default async function CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl">
      <Link href="/crm/yritykset" className="inline-flex items-center gap-1.5 text-sm text-ink-ghost hover:text-ink mb-5 transition-colors">
        <ChevronLeft size={15} />Yritykset
      </Link>

      <div className="mb-6">
        <h1 className="text-xl font-bold text-ink">{company.name}</h1>
        {company.business_id && <p className="text-sm text-ink-ghost mt-0.5">Y-tunnus: {company.business_id}</p>}
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-wire">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-ink-ghost uppercase tracking-wider">Nimi</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-ink-ghost uppercase tracking-wider hidden md:table-cell">Sähköposti</th>
                ...
                <tbody className="divide-y divide-wire/50">
                  {contacts.map((c) => (
                    <tr key={c.id} className="hover:bg-surface/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-ink-dim hidden md:table-cell">{c.email ?? "—"}</td>
                      <td className="px-4 py-3">
                        <span className={cn("inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border", STATUS_COLORS[c.status] ?? "bg-surface text-ink-ghost border-wire")}>
