...
export default async function LaskutusPage() {
  ...
  const { data: invoices } = await supabase
    .from("invoices")
    .select(
      "id, invoice_number, amount, status, due_date, paid_at, created_at, customers(id, first_name, last_name, email)",
    )
    .is("deleted_at", null)
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-elevated border border-wire rounded-xl p-5">
          ...
          <p className="text-2xl font-bold text-ok mt-2">
            {paid.toLocaleString("fi-FI")} €
          </p>
        </div>
        <div className="bg-elevated border border-wire rounded-xl p-5">
          <p className="text-xs text-ink-ghost font-medium uppercase tracking-wider">
          ...
          <p className="text-2xl font-bold text-copper mt-2">
            {open.toLocaleString("fi-FI")} €
          </p>
        </div>
        <div className="bg-elevated border border-wire rounded-xl p-5">
          <p className="text-xs text-ink-ghost font-medium uppercase tracking-wider">
          ...
          <p className="text-2xl font-bold text-bad mt-2">
            {overdue.toLocaleString("fi-FI")} €
          </p>
        </div>
      </div>
      ...
      <div className="bg-elevated border border-wire rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-wire flex items-center justify-between">
          ...
          <Link
            ...
            className="text-xs text-copper hover:text-copper/80 transition-colors"
          >
            Näytä kaikki →
          </Link>
        </div>
              ...
              {all.map((inv: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-ink hidden md:table-cell">
                      {customerName}
                    </td>
                    <td className="px-5 py-3 font-medium text-ink">
                      {inv.amount != null
                        ? `${Number(inv.amount).toLocaleString("fi-FI")} €`
                        : "—"}
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${STATUS_COLORS[inv.status] ?? "bg-surface text-ink-ghost border-wire"}`}
                      >
