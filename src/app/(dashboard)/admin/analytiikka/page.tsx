...
async function getStats() {
  ...
  const [
    ...
    supabase
      .from("projects")
      .select("name, status, progress_pct, created_at")
      .is("deleted_at", null)
...
const STATUS_COLORS: Record<string, string> = {
  planning: "bg-ink-ghost",
  development: "bg-copper",
  testing: "bg-teal-400",
  review: "bg-amber-400",
  completed: "bg-ok",
  cancelled: "bg-bad",
  active: "bg-ok",
  inactive: "bg-ink-ghost",
  lead: "bg-copper",
};
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="bg-elevated border border-wire rounded-xl p-5">
      <p className="text-xs text-ink-ghost font-medium uppercase tracking-wider">
        {label}
      </p>
    ...
    <div className="flex items-end gap-3 h-32 mt-4">
      {data.map((d) => (
        <div key={d.label} className="flex flex-col items-center gap-1 flex-1">
          ...
          />
          <span className="text-[10px] text-ink-ghost text-center leading-tight">
            {d.label}
          </span>
        </div>
      ))}
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-ink-ghost mt-1">
          Yleiskatsaus alustan tilastotietoihin
        </p>
      </div>
        ...
        <div className="bg-elevated border border-wire rounded-xl p-5">
          ...
          <div className="flex flex-col gap-3">
            {stats.recentProjects.length === 0 && (
              <p className="text-sm text-ink-ghost">Ei projekteja</p>
            )}
            {stats.recentProjects.map((p: any) => (
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full shrink-0 ${STATUS_COLORS[p.status] ?? "bg-ink-ghost"}`}
                />
                ...
                <div className="w-16 shrink-0">
                  ...
                  <p className="text-[10px] text-ink-ghost text-right mt-0.5">
                    {p.progress_pct ?? 0}%
                  </p>
                </div>
              </div>
      ...
      <div className="bg-elevated border border-wire rounded-xl p-5">
        ...
        <div className="flex flex-col divide-y divide-wire/50">
          {stats.recentCustomers.length === 0 && (
            <p className="text-sm text-ink-ghost py-2">Ei asiakkaita</p>
          )}
          {stats.recentCustomers.map((c: any) => (
            <div key={c.email} className="flex items-center gap-3 py-2.5">
              ...
              <span
                className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[c.status] === "bg-ok" ? "bg-ok/15 text-ok" : STATUS_COLORS[c.status] === "bg-copper" ? "bg-copper/15 text-copper" : "bg-wire text-ink-ghost"}`}
              >
