...
export function HinnoitteluContent() {
  ...
  const reasonsInView = useRevealInView(reasonsRef);
  const stepsInView = useRevealInView(stepsRef);
  const starterInView = useRevealInView(starterRef);
  const servicesInView = useRevealInView(servicesRef);
  const addOnsInView = useRevealInView(addOnsRef);
  const maintenanceInView = useRevealInView(maintenanceRef);
  const includedInView = useRevealInView(includedRef);
  const trustInView = useRevealInView(trustRef);
  const faqInView = useRevealInView(faqRef);
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <p className="text-ink-dim max-w-lg mx-auto">
              ...
              yrityksesi tarpeiden, ominaisuuksien ja projektin laajuuden
              mukaan.
            </p>
          </div>
          <motion.div
            ...
            <CardCarousel defaultIndex={1}>
              {STARTER_PACKAGES.map((pkg) => (
                <Link
                  ...
                  className={`p-6 rounded-xl border flex flex-col h-full hover:opacity-90 transition-opacity ${pkg.name === "Pro" ? "gradient-border-violet bg-elevated" : pkg.name === "Startti" ? "gradient-border-white bg-elevated" : "gradient-border bg-elevated shadow-glow"}`}
                >
                  ...
                  <div className="mb-3">
                    <span className="text-copper font-bold text-2xl">
                      {pkg.setup}
                    </span>
                  ...
                  <span className="flex items-center gap-1.5 text-sm font-medium text-copper">
          ...
          >
            {STARTER_PACKAGES.map((pkg) => (
              <motion.div key={pkg.name} variants={fadeUp}>
                <Link
                  href={`/yhteystiedot?palvelu=${pkg.slug}`}
                  className={`p-6 rounded-xl border flex flex-col h-full hover:opacity-90 transition-opacity ${pkg.name === "Pro" ? "gradient-border-violet bg-elevated" : pkg.name === "Startti" ? "gradient-border-white bg-elevated" : "gradient-border bg-elevated"}`}
                >
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-display font-bold text-ink text-xl">
                      {pkg.name}
            ...
            <CardCarousel defaultIndex={2}>
              {SERVICES_PRICING.map((svc) => (
                <div
                  key={svc.name}
                  onClick={() =>
                    router.push(`/yhteystiedot?palvelu=${svc.slug}`)
                  }
                  className={`p-6 rounded-xl border flex flex-col h-full cursor-pointer hover:opacity-90 transition-opacity ${svc.name === "Verkkosivut" ? "gradient-border-white bg-elevated" : svc.name === "AI-ratkaisut" ? "gradient-border-teal bg-elevated" : svc.name === "Verkkokauppa" ? "gradient-border bg-elevated" : "gradient-border-violet bg-elevated"}`}
                >
                  <div className="mb-4">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-wire/40">
                    ...
                    >
                      Lue lisää <ArrowRight size={13} />
                    </Link>
                    <span className="flex items-center gap-1 text-sm font-medium text-copper">
                      Pyydä tarjous <ArrowRight size={13} />
                    </span>
                  </div>
                </div>
              ))}
      ...
      <section className="py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="md:hidden">
            <CardCarousel defaultIndex={1}>
              {MAINTENANCE_TIERS.map((tier) => (
                <div
                  key={tier.name}
                  className={`p-6 rounded-xl border flex flex-col h-full ${tier.name === "Perus" ? "gradient-border-white bg-elevated" : tier.name === "Standardi" ? "gradient-border bg-elevated" : "gradient-border-violet bg-elevated"}`}
                >
                  {tier.highlight && (
          ...
          <motion.div
            ...
            className="hidden md:grid md:grid-cols-3 gap-6 max-w-3xl mx-auto"
          >
            {MAINTENANCE_TIERS.map((tier) => (
              <motion.div
                key={tier.name}
                variants={fadeUp}
                className={`relative p-6 rounded-xl border flex flex-col ${tier.name === "Perus" ? "gradient-border-white bg-elevated" : tier.name === "Standardi" ? "gradient-border bg-elevated" : "gradient-border-violet bg-elevated"}`}
              >
                {tier.highlight && (
                  <div className="flex justify-center mb-4">
                    <Badge variant="accent" className="ring-1 ring-copper/40">
      ...
      <section className="py-16 bg-surface/30">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <p className="text-ink-dim max-w-lg mx-auto">
              Tarvitsetko vain yhden ominaisuuden? Lisätään se sivustollesi
              ilman koko pakettia.
            </p>
          </div>
          <motion.div
      ...
      <section className="relative py-24 overflow-hidden bg-elevated border-t border-wire">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative text-center max-w-3xl">
          ...
          <p className="text-ink-dim text-lg leading-relaxed mb-8 max-w-xl mx-auto">
            Käymme yhdessä läpi yrityksesi tarpeet ja suosittelemme juuri
            sinulle sopivan ratkaisun ilman sitoutumista.
          </p>
