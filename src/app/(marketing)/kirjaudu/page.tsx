        ...
        <div className="w-full max-w-sm">
          <div className="bg-elevated border border-wire rounded-2xl p-8 shadow-2xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="font-display font-bold text-lg text-ink">Täydennä tietosi</h1>
          ...
          <div className="bg-elevated border border-wire rounded-2xl p-8 shadow-2xl">
            ...
            {stage === "phone" ? (
              <form onSubmit={sendOtp} className="flex flex-col gap-4">
                <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+358 40 123 4567" required className={inputClass} />
                {err && <p className="text-red-400 text-xs">{err}</p>}
                <button type="submit" disabled={loading || !phone} className="flex items-center justify-center gap-2 w-full px-5 py-3.5 rounded-xl bg-copper text-[#0A0C10] font-semibold text-sm hover:bg-copper-light transition-colors disabled:opacity-60">
                  {loading ? "Lähetetään..." : <>Lähetä koodi <ArrowRight size={15} /></>}
                </button>
              </form>
            ) : (
              <form onSubmit={verifyOtp} className="flex flex-col gap-4">
                <input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="123456" maxLength={6} required className={inputClass + " text-center tracking-widest text-lg"} />
                {err && <p className="text-red-400 text-xs">{err}</p>}
                <button type="submit" disabled={loading || otp.length < 6} className="flex items-center justify-center gap-2 w-full px-5 py-3.5 rounded-xl bg-copper text-[#0A0C10] font-semibold text-sm hover:bg-copper-light transition-colors disabled:opacity-60">
