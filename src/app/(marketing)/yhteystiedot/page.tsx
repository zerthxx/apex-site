...
const CONTACT_DETAILS = [
  {
    icon: Mail,
    label: "Sähköposti",
    value: "info@apexsite.fi",
    href: "mailto:info@apexsite.fi",
  },
  {
    icon: Phone,
    label: "Puhelin",
    value: "+358 44 2455490",
  ...
  return (
    <>
      ...
      <section className="py-16 md:py-24 lg:py-32">
        <Container>
          <RevealSection className="grid grid-cols-1 lg:grid-cols-5 gap-12">
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="space-y-4">
                  {CONTACT_DETAILS.map(({ icon: Icon, label, value, href }) => (
                    <div key={label} className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs text-ink-ghost uppercase tracking-wider mb-0.5">
                          {label}
                        </p>
                        {href ? (
                          <a
                            href={href}
                            className="text-ink hover:text-copper transition-colors font-medium text-sm"
                          >
                            {value}
                          </a>
                        ) : (
              ...
              <div className="p-5 rounded-xl bg-elevated border border-wire">
                <h3 className="font-heading font-semibold text-ink mb-3 text-sm">
                  Miten se etenee?
                </h3>
